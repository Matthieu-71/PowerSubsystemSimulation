...
function [outx, outy, outz] = zRot(vec,theta)
...
f.figure_position = [scrnSize(3)/2 0]; // Sets the figure at the top and towards the middle of the user's screen
a = gca()
...
for i = 1:(row*col)
    ...
    [newx(i) newy(i) newz(i)] = yRot([xIns(i),yIns(i),zIns(i)], ang);
    [newx(i) newy(i) newz(i)] = zRot([xIns(i),yIns(i),zIns(i)], ang);
end
